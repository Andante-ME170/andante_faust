...
*
* To use the code, keep the address of BOTH IMUs as 0x68 (do not pull ADO pin high),
...
void MIDIsetup() {
  ...
  Bluefruit.begin();
  Bluefruit.setName("Bluefruit52 MIDI 2");
...
//    Serial.print("\n");
    ...
    float kneeAngleBufferRange = 30.0f;
    float modVal = knee_x-kneeAngleBufferRange;
    
    modVal = max(0, modVal);
    modVal = min(90, modVal);
    modVal = map(modVal, 0, 90, 1, 100);  // scaling done in app
  
    //send new mod value if it has changed
    // TODO check within window
    if (abs(lastModVal - modVal) > 1 ) {
    ...
    // MIDI.sendNoteOn(int(knee_x), 100, 1);
    return true;
  } 
  return false;
}
